...
const NOTEBOOK_URL =
  "https://notebooklm.google.com/notebook/53536c72-fb38-4b28-a622-435079600f14";
...
export default function Sidebar({
  ...
  const navItems = [
    ...
    ...(user?.role === "admin"
      ? [{ id: "admin", icon: "⚙️", label: "Quản trị hệ thống" }]
      : []),
  ];
      ...
      <aside className={`sidebar ${open ? "open" : "closed"}`}>
        ...
        <div className="p-[18px_16px] border-b border-forest/10 bg-forest-light flex items-center gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-gold text-xs font-extrabold tracking-wide">
              CẨM NANG
            </div>
          ...
          {}
          <button
            className="hamburger-btn bg-white/10 border-none text-white/60 w-8 h-8 rounded-lg cursor-pointer text-base flex items-center justify-center hover:bg-white/20"
            onClick={onClose}
          >
            ✕
          </button>
        ...
        {user && (
          <div className="p-[12px_14px] border-b border-slate-100 flex items-center gap-2.5 bg-slate-50/50">
            <div className="w-[38px] h-[38px] rounded-full bg-forest flex items-center justify-center text-[15px] font-extrabold text-gold shrink-0 border border-gold/20">
              {user.full_name?.charAt(user.full_name.lastIndexOf(" ") + 1) ||
                "U"}
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-slate-800 text-xs font-bold truncate">
                {user.full_name}
              </div>
        ...
        <nav className="flex-1 p-[10px_8px] flex flex-col gap-0.5 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                className="nav-item flex items-center gap-3 p-[11px_14px] rounded-lg cursor-pointer transition-all border-none w-full text-left"
          ...
          >
            ...
            <span className="text-[10px] font-bold text-slate-400 group-hover:text-forest transition-colors">
              ↗
            </span>
          </button>
        </nav>
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
              ...
              <p className="m-0 text-sm text-amber-800 leading-relaxed font-medium">
                Tính năng này sử dụng <strong>Google NotebookLM</strong>. Bạn
                cần đăng nhập tài khoản Google trên trình duyệt trước khi truy
                cập.
              </p>
            </div>
            <p className="m-0 text-sm text-slate-500 leading-relaxed">
              Sổ tay AI tổng hợp và trả lời câu hỏi dựa trên toàn bộ văn bản
              pháp luật đã được nạp vào hệ thống.
            </p>
