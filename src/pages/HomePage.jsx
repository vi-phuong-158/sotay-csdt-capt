...
export default function HomePage({ setActiveNav }) {
  ...
  const filteredDocs = useMemo(() => {
    ...
    if (selectedCategory !== 'all') {
      ...
      result = result.filter(d => {
        ...
        return slug === selectedCategory || label === (catItem?.label || '').toLowerCase();
      });
    }
  ...
  const bookmarkedDocs = useMemo(
    () => bookmarks.map(key => documents.find(d => docKey(d) === key)).filter(Boolean),
    [bookmarks, documents]
  );
      ...
      {showNewBanner && (
        <div className="flex items-center gap-3 mb-6 bg-green-50 border border-green-200 rounded-xl p-4">
          ...
          <p className="m-0 flex-1 text-sm font-bold text-green-800">
            Có <span className="text-green-600">{newCountSnapshot}</span> văn bản mới được thêm kể từ lần đăng nhập trước.
          </p>
          <button
      ...
      {bookmarkedDocs.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-lg">★</span>
            <h2 className="text-sm font-bold text-slate-800 m-0 uppercase tracking-widest">Đã ghim</h2>
            <span className="text-xs text-slate-400 font-bold">({bookmarkedDocs.length})</span>
          </div>
            ...
            {bookmarkedDocs.map(doc => (
              ...
              >
                <button
                  ...
                  title="Bỏ ghim"
                >★</button>
                <div className="text-[11px] font-bold text-forest mb-1 uppercase tracking-wide">{doc.categoryLabel || 'Khác'}</div>
                <div className="text-[13px] font-bold text-slate-800 leading-snug line-clamp-3">{doc.title}</div>
                <div className="text-[10px] text-slate-400 font-bold mt-2">{doc.issue_number || '---'}</div>
              </div>
            ))}
          </div>
        </div>
