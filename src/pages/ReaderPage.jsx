    ...
    words.forEach((word, idx) => {
      ...
      if (cleanWord.includes(term) || term.includes(cleanWord)) {
        result.push(
          <mark
            key={idx}
            className="bg-gold/60 text-forest-dark font-bold px-0.5 rounded"
          >
            {word}
          </mark>,
          " ",
  ...
  return (
    <div className="min-h-full bg-slate-50 text-slate-900 relative pb-20 animate-fade-up">
      ...
      <div className="sticky top-0 z-50 bg-forest-light text-white p-3 md:p-4 flex items-center gap-3 shadow-lg">
        <button
          onClick={onBack}
          className="bg-white/10 border-none text-gold w-8 h-8 rounded-lg cursor-pointer flex items-center justify-center hover:bg-white/20 transition-all"
        >
          ←
        </button>
        <div className="flex-1 min-w-0 px-2">
          <h2 className="m-0 text-sm md:text-base font-bold truncate leading-tight">
            {doc.title}
          </h2>
          <div className="text-[10px] md:text-[11px] text-white/70 truncate">
            {doc.issue_number}
          </div>
        </div>
        <div className="flex gap-1.5 shrink-0">
          ...
          <button
            ...
            className="bg-white/10 border-none rounded-lg w-8 h-8 md:w-9 md:h-9 flex items-center justify-center text-white cursor-pointer font-bold hover:bg-white/20 transition-all"
          >
            A-
          </button>
          <button
            onClick={() => setFontSize((f) => Math.min(24, f + 1))}
            className="bg-white/10 border-none rounded-lg w-8 h-8 md:w-9 md:h-9 flex items-center justify-center text-white cursor-pointer font-bold text-lg hover:bg-white/20 transition-all"
          >
            A+
          </button>
        </div>
      ...
      <div className="max-w-[1000px] mx-auto">
        ...
        <div
          ref={contentRef}
          className="p-[24px_16px] md:p-12 bg-white min-h-screen shadow-sm"
        >
          <div className="mb-10 pb-8 border-b-4 border-gold/30 max-w-3xl mx-auto">
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 mb-6 leading-tight text-center">
              {doc.title}
            </h1>
