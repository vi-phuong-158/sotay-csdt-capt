...
function AdminPage() {
  ...
  const filteredLogs = useMemoA(() => {
    ...
    if (logSearch.trim())
      l = l.filter(
        (x) =>
          window.fuzzyMatch(x.full_name, logSearch) ||
          window.fuzzyMatch(x.username, logSearch) ||
          window.fuzzyMatch(x.details, logSearch),
      );
    return l;
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: "linear-gradient(135deg,#1a5c3d,#1e3a8a)",
          borderRadius: 12,
          padding: "20px 24px",
          marginBottom: 24,
          color: "#fff",
      ...
      >
        <div>
          ...
          <p style={{ margin: "4px 0 0", opacity: 0.75, fontSize: 13 }}>
            Quản lý tài khoản & giám sát hoạt động hệ thống
          </p>
        </div>
      ...
      >
        ...
        {tab === "users" && (
          <div>
            ...
            >
              Hiển thị {filteredUsers.length}/{users.length} tài khoản — còn{" "}
              {300 - users.length} slot trống
            </div>
            ...
            >
              <div
                style={{
                  background: "linear-gradient(90deg,#113a26,#1e3a8a)",
                  height: "100%",
                  width: (users.length / 300) * 100 + "%",
                  borderRadius: 4,
                      ...
                      <td style={{ padding: "12px" }}>
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            background:
                              u.role === "admin" ? "#7f1d1d33" : "#1e3a8a22",
                            color: u.role === "admin" ? "#dc2626" : "#1e3a8a",
                          }}
                        >
                ...
                return (
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        {doc.content.chapters.length} chương ·{" "}
                        {doc.content.chapters.reduce(
                          (s, c) => s + c.articles.length,
                          0,
                        )}{" "}
                        điều · Cập nhật: {doc.updatedAt}
                      </div>
                    </div>
                    <button
        ...
        {tab === "logs" && (
          <div>
            ...
            >
              Tổng {filteredLogs.length} nhật ký
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {filteredLogs.slice(0, 50).map((log) => {
                ...
                const docTitle =
                  log.action === "VIEW_DOC"
                    ? window.DOC_TITLES[log.details] || log.details
                    : log.details;
                return (
                  <div
                    key={log.id}
                  ...
                  >
                    ...
                    >
                      ...
                      {log.action === "LOGIN" && (
                        <span style={{ color: "#059669" }}>✓ {docTitle}</span>
                      )}
                    </div>
                    <span
                      style={{
      ...
      {}
      {showAddDoc && (
        <AddDocModal onClose={() => setShowAddDoc(false)} onAdd={addDocument} />
      )}
      ...
      {resetTarget && (
        ...
        >
          ...
          >
            <button onClick={() => setResetTarget(null)} style={btnSecondary}>
              Hủy
            </button>
            <button
              onClick={() => {
                if (newPass.trim().length >= 4) {
                  resetPassword(resetTarget.id, newPass.trim());
                  setResetTarget(null);
                } else showToast("Mật khẩu phải ít nhất 4 ký tự.", "error");
              }}
              style={btnPrimary}
            >
              Xác nhận
...
function AddDocModal({ onClose, onAdd }) {
  ...
  const [chapters, setChapters] = useStA([
    {
      ...
      articles: [{ id: "art1", title: "Điều 1. Phạm vi điều chỉnh", text: "" }],
    },
  ]);
  ...
  const addChapter = () => {
    ...
    setChapters((prev) => [
      ...prev,
      {
        id: "ch" + n,
        title: "Chương " + n + ": ",
        articles: [{ id: "art" + Date.now(), title: "Điều 1. ", text: "" }],
      },
    ]);
  };
  ...
  const updateChapter = (ci, val) =>
    setChapters((prev) =>
      prev.map((c, i) => (i === ci ? { ...c, title: val } : c)),
    );
  const addArticle = (ci) =>
    setChapters((prev) =>
      prev.map((c, i) =>
        i === ci
          ? {
              ...
              articles: [
                ...c.articles,
                { id: "art" + Date.now(), title: "Điều. ", text: "" },
              ],
            }
          : c,
      ),
    );
  const removeArticle = (ci, ai) =>
    setChapters((prev) =>
      prev.map((c, i) =>
        i === ci
          ? { ...c, articles: c.articles.filter((_, j) => j !== ai) }
          : c,
      ),
    );
  const updateArticle = (ci, ai, k, v) =>
    setChapters((prev) =>
      prev.map((c, i) =>
        i === ci
          ? {
              ...c,
              articles: c.articles.map((a, j) =>
                j === ai ? { ...a, [k]: v } : a,
              ),
            }
          : c,
  ...
  const handleSave = () => {
    if (!form.title.trim() || !form.issue_number.trim()) {
      alert("Vui lòng điền Tiêu đề và Số ký hiệu.");
      return;
    }
      ...
      >
        ...
        >
          <div>
            ...
            >
              Bước {step}/2 —{" "}
              {step === 1 ? "Thông tin cơ bản" : "Nội dung chương/điều"}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input
                  ...
                  placeholder="VD: Bộ luật Hình sự năm 2015..."
                  style={inputStyle}
                />
              </div>
              <div
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setF("issue_number", e.target.value)}
                    placeholder="VD: Luật số 100/2015/QH13"
                    style={inputStyle}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setF("updatedAt", e.target.value)}
                    style={inputStyle}
                  />
                </div>
              </div>
              <div>
                ...
                <select
                  value={form.category}
                  onChange={(e) => {
                    const opt = CAT_OPTIONS.find(
                      (c) => c.id === e.target.value,
                    );
                    setF("category", e.target.value);
                    setF("categoryLabel", opt?.label || "");
                  }}
                ...
                >
                  {CAT_OPTIONS.map((c) => (
                    <option
                      key={c.id}
                      value={c.id}
                      style={{ background: "#0d2d1e", color: "#FFD700" }}
                    >
                      {c.label}
                    </option>
                  ))}
              ...
              >
                ...
                <button
                  onClick={() => {
                    if (!form.title.trim() || !form.issue_number.trim()) {
                      alert("Điền Tiêu đề và Số ký hiệu.");
                      return;
                    }
                    setStep(2);
                  }}
            ...
            <div>
              ...
              >
                <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 13 }}>
                  ...
                  {chapters.reduce((s, c) => s + c.articles.length, 0)} điều
                </span>
                <button
                  onClick={addChapter}
                  style={{
                        ...
                        >
                          ...
                          >
                            <input
                              value={art.title}
                              onChange={(e) =>
                                updateArticle(ci, ai, "title", e.target.value)
                              }
                          ...
                          <textarea
                            value={art.text}
                            onChange={(e) =>
                              updateArticle(ci, ai, "text", e.target.value)
                            }
